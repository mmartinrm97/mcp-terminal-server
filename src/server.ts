...
const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        id: { type: 'string', description: 'Session ID.' },
        pattern: { type: 'string', description: 'Regex pattern to wait for (e.g. "package name:").' },
        timeout_ms: {
          type: 'number',
...
export function createTerminalServer(sessionManager: SessionManager): Server {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    return handleCallTool(sessionManager, { name, arguments: args as Record<string, unknown> | undefined });
  });
