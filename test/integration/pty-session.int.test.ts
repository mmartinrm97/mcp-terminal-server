...
function createSession(id: string, overrides?: Partial<{ cols: number; rows: number; cwd: string }>): PTYSession {
...
describe('PTYSession Integration', () => {
  ...
  describe('readUntil multi-line output', () => {
    it('should match a pattern in multi-line output', async () => {
      ...
      session.write(
        cmd(`node -e "console.log('MULTI-LINE-A');console.log('MULTI-LINE-B');console.log('MULTI-LINE-C')"\n`),
      );
  ...
  describe('close and force close', () => {
    ...
    it('should produce exit code after process terminates', async () => {
      ...
      try {
        session.close();
      } catch { /* may throw on Windows for signal-based close */ }
