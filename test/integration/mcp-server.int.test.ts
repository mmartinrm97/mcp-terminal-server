...
describe('MCP Server Integration', () => {
  ...
  describe('full session lifecycle via handlers', () => {
    it('should complete create → write → read_until → close cycle', async () => {
      ...
      // 1. CREATE session
      const createResult = await handleCallTool(
        sm as unknown as SessionManagerType,
        { name: 'terminal_create_session', arguments: { shell: 'auto' } },
      );
      ...
      const writeResult = await handleCallTool(
        sm as unknown as SessionManagerType,
        { name: 'terminal_write', arguments: { id: sessionId, data: 'echo ---MCP-FLOW-M1N2---\n' } },
      );
      const writeData = JSON.parse(writeResult.content[0].text) as { ok: boolean; bytes_written: number };
      expect(writeData.ok).toBe(true);
      expect(writeData.bytes_written).toBeGreaterThan(0);

      // 3. READ_UNTIL the marker appears
      const readUntilResult = await handleCallTool(
        sm as unknown as SessionManagerType,
        {
      ...
      // 4. CLOSE session
      const closeResult = await handleCallTool(
        sm as unknown as SessionManagerType,
        { name: 'terminal_close_session', arguments: { id: sessionId } },
      );
      const closeData = JSON.parse(closeResult.content[0].text) as { ok: boolean };
      expect(closeData.ok).toBe(true);

      // Verify session is gone
      const listResult = await handleCallTool(
        sm as unknown as SessionManagerType,
        { name: 'terminal_list_sessions', arguments: {} },
      );
  ...
  describe('read and resize via handlers', () => {
    it('should read accumulated output', async () => {
      ...
      await handleCallTool(
        sm as unknown as SessionManagerType,
        { name: 'terminal_write', arguments: { id: sessionId, data: 'echo ---READ-TEST-X1Y2---\n' } },
      );
      ...
      const readResult = await handleCallTool(
        sm as unknown as SessionManagerType,
        { name: 'terminal_read', arguments: { id: sessionId, flush: false } },
      );
      ...
      await handleCallTool(
        sm as unknown as SessionManagerType,
        { name: 'terminal_close_session', arguments: { id: sessionId, force: false } },
      );
    }, 20000);
    ...
    it('should resize session and confirm new dimensions', async () => {
      ...
      const createResult = await handleCallTool(
        sm as unknown as SessionManagerType,
        { name: 'terminal_create_session', arguments: { shell: 'auto', cols: 80, rows: 24 } },
      );
      const createData = JSON.parse(createResult.content[0].text) as { id: string };
      expect(createData.id).toBeTruthy();
      ...
      // Resize
      const resizeResult = await handleCallTool(
        sm as unknown as SessionManagerType,
        { name: 'terminal_resize', arguments: { id: sessionId, cols: 120, rows: 50 } },
      );
