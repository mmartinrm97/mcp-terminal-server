...
| GitHub Copilot CLI    | Verified   | MCP reachability works, and a real interactive `create-vite` PTY flow completed successfully with scaffold verification once `terminalize` was added to `~/.copilot/mcp-config.json` and tool execution was approved | Print-mode validation may still require explicit tool approval (`--allow-all-tools`) depending on the Copilot execution mode                       | Verified interactive flow with client-specific permission caveat |
...
| Antigravity CLI (AGY) | Verified   | AGY can discover the plugin, call `terminalize/terminal_ping`, and complete a real interactive `create-vite` PTY flow with final scaffold verification when installed from `~/.gemini/config/plugins/terminalize`    | Local auth/plugin state should still be validated first if AGY starts in a stale or logged-out state                                               | Verified interactive flow with local AGY setup caveat            |
| OpenCode              | Partial    | MCP reachability validated with `terminal_ping`; CLI run successfully invoked `terminalize_terminal_ping` and returned `PING_OK`                                                                                     | A full interactive PTY workflow has not been re-verified yet under the stricter evidence bar used in this matrix                                   | Healthy MCP bridge, interactive proof still missing              |
| Pi                    | Verified   | Pi works through `pi-mcp-adapter` plus shared MCP config in `~/.config/mcp/mcp.json`, and a real interactive `npm init` PTY flow completed with final `package.json` verification                                    | Pi prefers shared MCP files and can fail with `Server terminalize has no command or url` if a Pi-owned override shadows the base server definition | Verified interactive flow with configuration-sensitive setup     |
...
- Antigravity requires the shared Gemini plugin path (`~/.gemini/config/plugins/terminalize`) and a healthy AGY login/plugin state before validation.
